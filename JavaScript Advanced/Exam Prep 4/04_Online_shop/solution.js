function onlineShop(selector){    
    ...
    let displayList = document.querySelector("ul.display");
    ...
    submitBtn.onclick = function(e){
        ...
        if(name.toString().length > 0 && price.toString().length > 0 && quantity.toString().length > 0){
            ...
            let listChildren = Array.from(displayList.children);
            if(listChildren.length >= 150){
                ...
                let capacityField = document.querySelector("#capacity");
                capacityField.value = listChildren.length;
                
                let totalPrice = 0;

                Array.from(listChildren).forEach((ch) => {
